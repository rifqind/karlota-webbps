<template>
  <tbody ref="tableRef">
    <tr>
      ...
      <template v-for="yr in props.yearsToRender" :key="'krt-' + yr">
        <td v-for="(node, index) in quarters" class="text-right">
          <!-- {{ getData("krt", node.label) }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["krt"]?.[String(yr)]?.q?.[Number(node) - 1] ?? 0,
              0,
              props.toFixed
            )
          }}
        ...
        <td class="text-right">
          <!-- {{ getSumTotalFromVal("krt") }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["krt"]?.[String(yr)]?.total ?? 0,
              0,
              props.toFixed
            )
          }}
    ...
    <tr>
      ...
      <template v-for="yr in props.yearsToRender" :key="'kap-' + yr">
        <td v-for="(node, index) in quarters" class="text-right">
          <!-- {{ getData("kap", node.label) }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["kap"]?.[String(yr)]?.q?.[Number(node) - 1] ?? 0,
              0,
              props.toFixed
            )
          }}
        ...
        <td class="text-right">
          <!-- {{ getSumTotalFromVal("kap") }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["kap"]?.[String(yr)]?.total ?? 0,
              0,
              props.toFixed
            )
          }}
    ...
    <tr>
      ...
      <template v-for="yr in props.yearsToRender" :key="'pmtb-' + yr">
        <td v-for="(node, index) in quarters" class="text-right">
          <!-- {{ getData("pmtb", node.label) }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["pmtb"]?.[String(yr)]?.q?.[Number(node) - 1] ?? 0,
              0,
              props.toFixed
            )
          }}
        ...
        <td class="text-right">
          <!-- {{ getSumTotalFromVal("pmtb") }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["pmtb"]?.[String(yr)]?.total ?? 0,
              0,
              props.toFixed
            )
          }}
    ...
    <tr>
      ...
      <template v-for="yr in props.yearsToRender" :key="'lainnya-' + yr">
        <td v-for="(node, index) in quarters" class="text-right">
          <!-- {{ getData("lainnya", node.label) }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["lainnya"]?.[String(yr)]?.q?.[Number(node) - 1] ?? 0,
              0,
              props.toFixed
            )
          }}
        ...
        <td class="text-right">
          <!-- {{ getSumTotalFromVal("lainnya") }} -->
          {{
            formatNumberGerman(
              tableModel.rows?.["lainnya"]?.[String(yr)]?.total ?? 0,
              0,
              props.toFixed
            )
          }}
...
const tableModel = computed(() => {
  ...
  for (const sets of ["krt", "kap", "pmtb", "lainnya"]) {
    const key = `${sets}`;
    model.rows[key] = {};
  ...
  model.footer["PDRB"] = {};
  for (const y of years)
    model.footer["PDRB"][y] = sums(y, ["krt", "kap", "pmtb", "lainnya"]);
  return model;
});
watch(
  () => tableModel.value,
