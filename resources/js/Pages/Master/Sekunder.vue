      ...
      <table class="table border-2 mb-2 w-full" ref="tabelUser" id="tabel-user">
        ...
        <tbody>
          ...
          >
            ...
            <td class="align-middle">
              <span v-for="tahun in data.tahun" class="badge badge-info mr-1">{{
                tahun
              }}</span>
            </td>
            ...
            <td class="text-center align-middle deleted">
              <Link
                :href="route('master.sekunder.update', { id: data.id })"
                class="edit-pen mx-1"
              >
...
const ArrayBigObjects = [
  ...
  { key: "created_time", valueFilter: searchUpdated },
...
const fetchData = async () => {
  try {
    const response = await axios.get(route("master.sekunder.index"), {
      params: {
        ...
        ArrayFilter: {
          ...
          created_at: searchUpdated.value,
...
const deleteSubmit = async () => {
  try {
    ...
    form.delete(route("master.sekunder.destroy", { id: form.id }), {
      onSuccess: (response) => {
